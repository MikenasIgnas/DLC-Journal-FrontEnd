...
import { ColocationDataType, CompaniesType, UserType }  from '../../../types/globalTypes'
...
const VisitRegistrationForm = ({setCurrent}: VisitRegistrationFormPRops) => {
  ...
  const [selectedCompany, setSelectedCompany] =     React.useState<SingleCompanyType | undefined>()
  const [selectedSite, setSelectedSite] =           React.useState<SelectedSiteType[]>()
  const [form] =                                    useForm()
  ...
  const dlcEmployeeNames = dlcEmployees?.map((el) => {
            ...
            {selectedSite?.map((entry, index) => (
              <div key={index}>
                {Object.keys(entry).map(key => (
                  <div key={key}>
                    <strong>{key}:</strong>
                    {entry[key].map((value, innerIndex) => (
